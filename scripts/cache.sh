...
KOOKOO_API_KEY=<your-kookoo-api-key>
...
for i in `find $1/en -name "*.wav" `; do 
 ...
 curl $KOOKOO_URL/api_key/$KOOKOO_API_KEY/url/http:%5C%5C$TAMA_HOSTNAME%5Ctama%5Cwav%5Cstream%5Cen%5C$wave_file_name/delete/true
 echo "Uploading...en:$wave_file_name"
 curl -d "api_key=$KOOKOO_API_KEY&url=http://$TAMA_HOSTNAME/tama/wav/stream/en/$wave_file_name" $KOOKOO_URL
done
...
for i in `find $1/mr -name "*.wav" `; do 
 ...
 curl $KOOKOO_URL/api_key/$KOOKOO_API_KEY/url/http:%5C%5C$TAMA_HOSTNAME%5Ctama%5Cwav%5Cstream%5Cmr%5C$wave_file_name/delete/true
 echo "Uploading...mr:$wave_file_name"
 curl -d "api_key=$KOOKOO_API_KEY&url=http://$TAMA_HOSTNAME/tama/wav/stream/mr/$wave_file_name" $KOOKOO_URL 
done
